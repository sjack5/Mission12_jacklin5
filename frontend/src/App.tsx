...
const Teams = [   //This is my array of json objects
...
function Welcome() {    //Welcome message to tell people what site is about
  return(
  <div>
      <h1>Welcome to my Mission 12 Assignment</h1>
      <p>This is a place to check out all of the teams that are participating in March Madness this year and some more information on them!</p>
  </div>
  )
}

class Card extends React.Component<{ school: string, name: string, city: string, state: string}> {  //Not sure what this first line does but it fixes errors for me
  render(){
    
    const oneTeam = this.props;   //Allows us to grab instance of oneTeam from. Lists out school, name, city, state and puts them all in their own div tag
...
function TeamList(){    //This provides us a list of all the  teams in the array. Splits each object in the array into its individual elements for us to use later
  return(
  <div>
      {Teams.map(oneTeam => <Card {...oneTeam}/>)}   
  </div>
  );
}
...
function App() {    //Default app that we use to run our code and call our functions
  return(
    <div>
      <Welcome/>
      <TeamList/>
